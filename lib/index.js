...
// Matches the leading text of a `dead-or-alive` "dead" message so we can
// extract the HTTP status code without depending on internal fields.
const notOkResponseExpression = /^Unexpected not ok response `(\d+)`/
...
async function rule(tree, file, options) {
  ...
  await limit(async function () {
    ...
    const skipStatusCodes = settings.skipStatusCodes
      ? new Set(settings.skipStatusCodes)
      : undefined
        ...
        return async function () {
          ...
          for (const node of nodes) {
            for (const message of result.messages) {
              if (skipStatusCodes) {
                const match = notOkResponseExpression.exec(message.reason)

                if (match && skipStatusCodes.has(Number(match[1]))) {
                  continue
                }
              }
